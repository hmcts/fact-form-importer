...
def test_build_read_only_approval_users_deduplicates_and_excludes(tmp_path):
    ...
    submissions = [
        _submission(2, "Person@example.test", "Person Example"),
        _submission(50, " person@example.test ", "Ignored Later Name"),
        _submission(51, "team.member@example.test", "Team Member"),
...
def test_write_submitter_outputs_writes_json_and_workbook(tmp_path):
    ...
    submissions = [_submission(2, "person@example.test", "Person Example")]
    ...
    assert payload["users"][0]["email"] == "person@example.test"

    workbook = load_workbook(result.workbook_path)
    assert workbook.sheetnames == ["Read only approval users", "Excluded users"]
    assert workbook["Read only approval users"]["A2"].value == "person@example.test"
